...
mcp__playwright__browser_navigate({ url: "https://feature-xyz.pedicab512.pages.dev" })
...
- Email: `alexgarcia042@gmail.com`
- Password: `bigmoney` (all lowercase, all one word)

```typescript
// Find and fill login form
mcp__playwright__browser_click({ element: "login button", ref: "..." })
mcp__playwright__browser_type({ element: "email input", ref: "...", text: "alexgarcia042@gmail.com" })
...
BRANCH_URL="https://feature-{branch-name}.pedicab512.pages.dev"
...
When `SEND_TEST_EMAILS=true`, the custom Playwright reporter automatically sends ONE email to `alexgarcia042@gmail.com` containing:
...
SEND_TEST_EMAILS=true \
  ...
  PLAYWRIGHT_BASE_URL=https://feature-{branch}.pedicab512.pages.dev \
...
wrangler pages deployment list --project-name pedicab512 | grep "feature/{branch-name}" | head -1
...
wrangler pages deployment tail {FULL-UUID} --project-name pedicab512 --format pretty
...
```bash
PLAYWRIGHT_BASE_URL=https://feature-{branch}.pedicab512.pages.dev npm run test -- {test-file}
```
...
PLAYWRIGHT_BASE_URL=https://feature-multi-agent-team-cha.pedicab512.pages.dev \
...
- Authenticates with alexgarcia042@gmail.com (test admin)
...
Latest: https://feature-multi-agent-team-cha.pedicab512.pages.dev
...
wrangler pages deployment list --project-name pedicab512 | grep "feature/multi-agent-team-chat" | head -1
# → 773fe384-05b8-4e12-8773-19f6e7e68336

# 2. Start tailing (background)
wrangler pages deployment tail 773fe384-05b8-4e12-8773-19f6e7e68336 --project-name pedicab512 --format pretty
# → bash_id: abc123

# 3. Run test
PLAYWRIGHT_BASE_URL=https://feature-multi-agent-team-cha.pedicab512.pages.dev npm run test -- backroom-team-chat --grep "should send message"
